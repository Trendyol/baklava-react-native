...
Linters that we use are [`eslint`](https://eslint.org) and
...
`stylelint` run for `css` files. Also we check commit message with `commitlint`. If there are any issues commit will be
